  ...
  methods: {
      ...
      for (let i = 1; i <= 3; i++) {
        for (let j = 1; j <= 3; j++) {
          ...
          if (offset > 0) {   // 横屏
            ...
            lockCicle.y = (this.width / 4) * i + Math.abs(offset) - this.height / 5;
            lockCicle.state = 0;
            lockCicle.stateShow = 0;
          }
    ...
    drawLinePointer (x, y, flag) {
      ...
      for (var i = 0; i < this.arr.length; i++) {
        ...
        if (this.ctx.isPointInPath(x, y) && this.currentPointer != i && this.puts.indexOf(i + 1) < 0) {
          ...
          this.arr[i].state = 1;
          if (!this.setAgain) {   // 第二次设置手势，小九宫格不再做相应改变
      ...
      this.canvas.addEventListener('touchend', function (e) {
        ...
        if (_this.setOrCheck) {
          if (_this.puts.length >= 4) {
            ...
            if (_this.setAgain) {
              // 第二次设置手势                   
              if (JSON.stringify(_this.puts) == JSON.stringify(_this.patternPassWord)) {
          ...
          if (_this.puts.length >= 4) {
            ...
            console.log('你输入的密码：', JSON.stringify(_this.puts), '实际密码：', JSON.stringify(_this.passWord))
            _this.init(true);
