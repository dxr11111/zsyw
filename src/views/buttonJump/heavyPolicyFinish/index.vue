    ...
    <div>
      <div class="panel">
        <div class="panel-item">
          <span>重保车次</span>
          <input type="text" v-model="formData.zbZbcc" />
        </div>
        <div class="panel-item">
          <span>重保人次</span>
          <input type="text" v-model="formData.zbZbrc" />
        </div>
        <div v-if="sheetType == 1">
          <div class="panel-item">
...
export default {
  ...
  methods: {
    async submit () {
      if (this.formData.custInfo == '') return this.$toast('请输入客户重保电路资料核实')
      if (this.sheetType == 1) {
        if (this.formData.isCheck == null) return this.$toast('请选择设备、线路是否预检预修')
        if (this.formData.isHasWarning == null) return this.$toast('请选择重保电路是否加入网管预警')
        if (this.formData.hasWarning == null) return this.$toast('请选择是否有告警')
        if (this.formData.impFlag == null) return this.$toast('请选择是否重保电路')
        if (this.formData.spareParts == '') return this.$toast('请输入备品备件')
        if (this.formData.linkName == '') return this.$toast('请输入联系人')
        if (this.formData.zblinkTel == '') return this.$toast('请输入联系人电话')
        if (this.formData.impRemark == '') return this.$toast('请输入落实情况备注')
      }
      if (this.isOverTime == 1) {
        if (this.formData.overTiemContent == '' || this.formData.overTiemContent == undefined) {
      ...
      console.log('参数', JSON.stringify(this.formData))
    ...
    async getBeforeFinish () {
      let data = await BeforeFinishApi(JSON.stringify({ id: Number(this.$route.query.id) }))
      console.log(data)
      this.formData = {
        zbZbcc: 0,
        zbZbrc: 0,
