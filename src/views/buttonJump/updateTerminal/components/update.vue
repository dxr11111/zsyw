...
<script>
import { JndSpecLineRouteTerminalApi, JndUpdateerminalApi } from '@/http/button'
...
export default {
  ...
  data () {
    return {
      ...
      reasonList: [{ name: '故障', value: '0' }, { name: '异常占用', value: '1' }],
      showReason: false,
    ...
    currDeviceId (newId, oldId) {
      ...
      this.moduleNamesSource.forEach(item => {
        if (item.parentId == newId) {
          this.moduleNames.push({ name: item.childName, id: item.childId, value: item.parentId })
        }
      })
  ...
  methods: {
    onSubmit () {
      ...
      this.params = {
        ...
        houShebeilxId: this.deviceNames.find(e => e.name == this.currDeviceName).id, //	后设备类型ID
        houShebeiId: this.deviceNames.find(e => e.name == this.currDeviceName).value, //	后设备ID
        houDuanZiId: str, //	后端子ID（格式如：17/18）
        houMokuaiId: this.moduleNames.find(e => e.name == this.currModuleName).id, //	后模块ID
        caoZuoYuan: getItem('loginNo'), // 操作员
        duankouMiaoshu: this.reasonList.find(e => e.name == this.currReason).value, // 修改说明
        gaixinYuanyin: this.intro, //修改原因
      }
    ...
    async getSpeclineRoute () {
      let data = await JndSpecLineRouteTerminalApi(JSON.stringify({
        shebeiLeiXingId: this.currData.shebeiLeiXingId,
        weiZhiId: this.currData.weiZhiId,
        weizhiMiaoShu: this.currData.weizhiMiaoShu,
      }))
      // console.log(data)
      this.deviceNames = data.list1.map(e => ({ name: e.childName, id: e.childId, value: e.parentId }))
