...
import { reqIomNewFinishQuery, reqIomNewSpeedForm, reqIomNewLanInQuire, reqIomNewWifiVm } from '@/http/button'
...
export default {
  ...
  methods: {
    ...
    clickIptvItem (index) {
      ...
      this.tempDevInfo = JSON.parse(JSON.stringify(this.postData.iptvInfoList[index]))
    },
    // 选中iptv设备信息-是否电视芯
    clickIsTelevisionCore (bool) {
      if (bool) {
        this.$set(this.postData.iptvInfoList[this.iptvPopIndex], 'isTelevisionCore', '1')
      } else {
        this.$set(this.postData.iptvInfoList[this.iptvPopIndex], 'isTelevisionCore', '0')
    ...
    confirmIptvPop () {
      ...
      if (iptvItem.isTelevisionCore === '1') {
        ...
        // 将值渲染在页面上
        this.iptvList[this.iptvPopIndex].televisionCore = iptvItem.televisionCore
      }
    ...
    cancelIptvPop () {
      this.iptvPopShow = false
      this.$set(this.postData.iptvInfoList, this.iptvPopIndex, this.tempDevInfo)
    },
    ...
    clickPjzItem (index) {
      ...
      this.tempDevInfo = JSON.parse(JSON.stringify(this.postData.pjzList[index]))
    },
    // 确认皮基站设备信息弹出层
    confirmPjzPop () {
    ...
    clickOttItem (index) {
      ...
      this.tempDevInfo = JSON.parse(JSON.stringify(this.postData.ottList[index]))
    },
    // 确认ott设备信息弹出层
    confirmOttPop () {
    ...
    confirmWifiPop () {
      ...
      if (wifiItem.isWifiBuild === '0') {
        ...
        this.wifiList.forEach((item, index) => {
          ...
          // 同步设置其他设备不施工原因
          this.$set(this.wifiList[index], 'wifiDescribe', wifiItem.wifiDescribe)
        })
        // 隐藏组网wifi设备测速内容
    ...
    clickFttrItem (index) {
      ...
      this.tempDevInfo = JSON.parse(JSON.stringify(this.postData.fttrInfo[index]))
    },
    // 确认fttr设备信息弹出层
    confirmFttrPop () {
      ...
      if (fttrItem.actType === 'A') {
        ...
        // 换机
        if (fttrItem.hasOwnProperty('sn') && fttrItem.hasOwnProperty('mac') && fttrItem.hasOwnProperty('oldSn') && fttrItem.hasOwnProperty('oldMac')) {
          if (fttrItem.sn == '' || fttrItem.mac == '' || fttrItem.oldSn == '' || fttrItem.oldMac == '') {
      ...
      if (fttrItem.isFttrBuild === '0') {
        this.$toast('您已选择不装该设备，但您填写了设备信息，系统已将该设备设置为安装，如需不安装，可手工将其设置为不装')
      }
      // 自动选中装按钮
    ...
    clickEquItem (equIndex, index) {
      ...
      this.tempDevInfo = JSON.parse(JSON.stringify(this.allEqu[equIndex].equipmentList[index]))
    },
    // 确认看家服务信息弹出层
    confirmEquPop () {
      let equItem = this.allEqu[this.allEquPopIndex].equipmentList[this.equPopIndex]
      // 判断是装还是不装
    ...
    cancelEquPop () {
      this.equPopShow = false
      this.$set(this.allEqu[this.allEquPopIndex].equipmentList, this.equPopIndex, this.tempDevInfo)
    },
    ...
    // 发起掌沃通测速
    async speedForm () {
      let result = await reqIomNewSpeedForm(JSON.stringify({ id: this.id, areaCode: this.areaCode }))
    ...
    judgeFinish () {
      ...
      // 判断固定ip
      if (this.IomNewFinishQuery.guDingIPtoWork == '99' && guDingIPisWork == '') {
          ...
          if (iptvUnworkedFlag === '0') {
            ...
            for (let i = 0; i < iptvInfoList.length; i++) {
              if (iptvInfoList[i].isTelevisionCore == '0') {
                // 选择电视芯
                if (iptvInfoList[i].televisionCore === '') return this.$toast('请填写iptv设备信息')
              } else {
                // 选择终端设备
                if (iptvInfoList[i].iptvStbid === '' && iptvInfoList[i].iptvMac === '') {
            ...
            // 选择不施工
            iptvInfoList = []
            if (iptvUnworkedBackReason == '') return this.$toast('请选择IPTV不施工原因')
          }
        } else {
          // 无法选择是否施工按钮-必须施工
          // 若item sn和mac都为空或电视芯串码为空则无法提交
          iptvInfoList.forEach(item => {
            if ((item.iptvStbid === '' && item.iptvMac === '') || item.televisionCore === '') {
      ...
      for (let i = 0; i < ottList.length; i++) {
        if (ottList[i].sn === '' && ottList[i].mac === '') return this.$toast('请填写ott设备信息')
      }

      // 判断沃家产品设备
      // 判断沃家产品是否填写施工内容
      for (let i = 0; i < wjcpList.length; i++) {
        if (wjcpList[i].sn === '' && wjcpList[i].mac === '') return this.$toast('请填写沃家产品设备信息')
      ...
      if (this.IomNewFinishQuery.isHaveWifiList === 1) {
        ...
        if (wifiList[0].isWifiBuild === '1') {
          ...
          for (let i = 0; i < wifiList.length; i++) {
            if (wifiList[i].sn === '' && wifiList[i].mac === '') return this.$toast('请填写WIFI产品设备施工信息')
          }
          // 是否操作WIFI终端 0：是  1：否
          iomIsOpWifi = '0'
        }
        ...
        if (this.listDetail.isFttrSelect === '1') {
          ...
          if (isFttrBuild === '') {
            ...
            for (let i = 0; i < fttrInfo.length; i++) {
              ...
              if (fttrInfo[i].isFttrBuild !== '0' && fttrInfo[i].isFttrBuild !== '1') {
                // 未选择
                ...
                if (fttrInfo[i].isFttrBuild === '1') {
                  ...
                  if (fttrInfo[i].actType === 'A') {
                    ...
                    if (fttrInfo[i].sn.length === 0 || !(fttrInfo[i].hasOwnProperty('sn'))) {
                      // 只选中是否施工，未点开填值
          ...
          for (let j = 0; j < item.length; j++) {
            ...
            if (item[j].isBuild === '') {
              ...
              if (item[j].actType === 'A' && item[j].sn.length === 0) {
                return this.$toast(`${this.allEqu[i].allName}设备选择装机，则SN、MAC不能为空`)
              } else if (item[j].actType === 'X' && item[j].sn.length === 0) {
                return this.$toast(`${this.allEqu[i].allName}设备选择换机，则新、旧设备SN、MAC不能为空`)
      ...
      if (this.IomNewFinishQuery.reasonLosFlag === 2) {
        if (iom_TestRate_LosInfo == '') return this.$toast('请选择ONU收光率强制回单原因')
      }
      // 判断光猫wifi测速
      if (wifiBackResonInfo == '') return this.$toast('请选择光猫WIFI测速强制回单原因')
      // 判断lan口协商速率
      if (this.IomNewFinishQuery.lanStatus == '3' || this.IomNewFinishQuery.lanStatus == '5') {
        if (this.lanRateReasonInfo == '') return this.$toast('请选择LAN口协商速率强制回单原因')
        if (this.lanRateDesc == '') return this.$toast('请输入LAN口协商速率强制回单说明')
      }
      // 判断组网wifi设备测速
      ...
      if (this.hideWifiZWRegion) {
        if (this.IomNewFinishQuery.wifiZwStatus == '2' || this.IomNewFinishQuery.wifiZwStatus == '4') {
          if (this.wifiZwBackResonInfo == '') return this.$toast('请选择组网WIFI设备测速强制回单原因')
          if (this.wifiZwBackExplain == '') return this.$toast('请输入组网WIFI设备测速强制回单说明')
        }
      }
