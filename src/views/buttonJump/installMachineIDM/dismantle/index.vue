...
export default {
  ...
  computed: {
    ...
    headName () {
      return `拆机(${this.listDetail.orderId})`
    }
  },
  methods: {
    // 回退
    ...
    processInfoList (infoList) {
      ...
      infoList.forEach(item => {
        if (item.zeroconfigStatus === 6 || item.zeroconfigStatus === 10 || item.zeroconfigStatus === 11 || item.zeroconfigStatus === 13 || item.zeroconfigStatus === 15) {
          // 新增中文
          let newButtonKeys = []
    ...
    async clickbuttonKey (btn, info) {
      ...
      switch (btn.key) {
        ...
        case ('again'):
          // 重试
          this.$dialog.confirm({
            message: `${info.zeroconfigStageName}环节，是否确定重试？`,
            className: 'myDialog',
            getContainer: '.diamante',
          }).then(async () => {
            // 发送请求
            let again = await reqIomNewZeroConfigAgain(JSON.stringify({ id, sheetNo, taskNo, zeroconfigStage, zeroconfigStageName }))
        ...
        case ('turnPerson'):
          // 转人工
          this.$dialog.confirm({
            message: `${info.zeroconfigStageName}环节，是否确定转人工？`,
            className: 'myDialog',
            getContainer: '.diamante',
          }).then(async () => {
            // 发送请求
            let turnPerson = await reqIomNewZeroConfigTurnPerson(JSON.stringify({ id, sheetNo, taskNo, zeroconfigStage, zeroconfigStageName }))
        ...
        case ('addInstall'):
          ...
          this.$dialog.confirm({
            ...
            let addInstallResult = await reqIomNewAddInstall(JSON.stringify({ id, baseNo, zeroconfigStageName }))
        ...
        case ('confirm'):
          ...
          this.$dialog.confirm({
            title: '确认已断电',
            message: '新装设备入网超过30分钟，需拆除重新入网，清除设备前请把ONU设备断电，确认已断电？',
            className: 'myDialog',
            getContainer: '.diamante',
          }).then(async () => {
            // 发送清除业务数据的接口
            let confirmResult = await reqIomNewDeleteData(JSON.stringify({ id, sheetNo, taskNo, zeroconfigStageName }))
        ...
        case ('changePhyNo'):
          ...
          this.$dialog.confirm({
            ...
            // 发送更改物理号的接口
            let changePhyNoResult = await reqIomNewChangePhyNo(JSON.stringify({ id, sheetNo, zeroconfigStageName }))
    ...
    async submitContinueWork () {
      ...
      // 提交接口
      let postData = { id, sheetNo, taskNo, zeroconfigStage, zeroconfigStageName, backResonId, backResonInfo, receiptDesc }
      let result = await reqIomNewZeroConfigGoOnWroking(JSON.stringify(postData))
    ...
    async confirmNewInstall () {
      ...
      if (this.newInstall.username !== loginNo || this.newInstall.password !== userPwd) {
      ...
      let taskNo = this.currentInfo.taskNo
      let zeroconfigStageName = this.currentInfo.zeroconfigStageName
      let result = await reqIomNewDeleteData(JSON.stringify({ id, sheetNo, taskNo, zeroconfigStageName }))
    ...
    async submitReject () {
      ...
      let zeroconfigStageName = this.currentInfo.zeroconfigStageName
      let result = await reqIomNewZeroConfigReject(JSON.stringify({ id, sheetNo, intro, zeroconfigStageName }))
    ...
    getBottomStage (infoList) {
      for (let i = 0; i < infoList.length; i++) {
        if (infoList[i].zeroconfigStage === 0) {
          // 零配置成功
          this.isAutoRefresh = '0'
          return this.bottomStage = 0
        }
        if (infoList[i].zeroconfigStage === 2) {
          // return this.bottomStage = 2
        }
        if (infoList[i].zeroconfigStage === 3) {
          // zeroconfigStage=3 → 拆机成功 → 无需自动刷新
          this.isAutoRefresh = '0'
          this.refreshCounDown = 10
          this.isAutoRefresh = this.oldRadio = '0'
    ...
    // 进度查询
    async getQueryProgress () {
      let result = await reqIomNewZeroConfigQueryProgress(JSON.stringify({ id: this.listDetail.id }))
    ...
    judgeShowFlag () {
      if (this.$route.query.showFlag == 2) {
        ...
        // 开启倒计时
        this.countDownTimer();
      } else if (this.$route.query.showFlag == 1) {
        // 已做拆机 → 使用拆机前查询的infoList渲染页面
        // 更新页面
        this.processInfoList(this.$route.query.zeroConfigInfoList)
        // 判断底部配置页面
        this.getBottomStage(this.$route.query.zeroConfigInfoList);
        this.countDownTimer();
