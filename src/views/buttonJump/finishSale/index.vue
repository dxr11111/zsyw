...
export default {
  ...
  methods: {
    async confirm() {
      ...
      if (this.userSituation == '30') {
        ...
        if (this.businessList.length == 0 && this.deviceList.length == 0) {
          return this.$toast('"业务信息"和"设备信息"不能同时为空，至少要填写一项业务信息或设备信息')
        }
        if (this.businessList.length > 0) {
          // 用户填写的是业务信息
          for (let i = 0; i < this.businessList.length; i++) {
            const ele = this.businessList[i]
            if (ele.newBusiNessName == '' && ele.newBusinessPrice == '') return this.$toast('业务信息中的业务名称和业务资费不能同时为空')
          }
        }
        ...
        if (this.businessList.length > 0 || this.deviceList.length > 0) {
          ...
          if (this.isContact == '1' && this.currDate == null) return this.$toast('请选择联系用户时间')
        }
      } else {
    ...
    delItem(type, index) {
      type == 'bus' ? this.businessList.splice(index, 1) : this.deviceList.splice(index, 1)
    },
    chooseTime(value) {
