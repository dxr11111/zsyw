  ...
  <div class="ifmFinish">
    ...
    <div class="main">
      <template v-if="hasWorkType == false">
        ...
        <!-- 铁塔是否按时上站 -->
        <div class="region" v-if="ttStation == 2">
          <div class="selectButton">
        ...
        <template v-if="isMainDept == '1'">
          ...
          <div class="region">
            <van-field
              ...
              v-model="excludeTime"
              placeholder="请选择故障恢复时间"
              readonly
              class="textRight"
              @click="excludeTimeShow = true"
            />
            <!-- 选择处理时间 -->
            <van-popup
              v-model="excludeTimeShow"
              position="bottom"
              :style="{ height: '40%' }"
            >
              <van-datetime-picker
                v-model="excludeTimeCurrent"
                type="datetime"
                :min-date="minDate"
                :max-date="maxDate"
                :formatter="formatter"
        ...
        >
          <van-field
            ...
            readonly
            class="textRight"
            @click="processTimeShow = true"
          />
          <!-- 选择处理时间 -->
          <van-popup
            v-model="processTimeShow"
            position="bottom"
            :style="{ height: '40%' }"
          >
            <van-datetime-picker
              v-model="dealDateCurrent"
              type="datetime"
              :min-date="minDate"
              :max-date="maxDate"
              :formatter="formatter"
        ...
        <!-- 问题验证结果 -->
        <div class="region" v-if="balkSubSortId21Show || balkSubSortId22Show">
          <van-field
        ...
        <template v-if="isXiaoJiao">
          ...
          <div class="region">
            <van-field
              ...
              is-link
              arrow-direction="down"
              v-model="xiaoJiaoReserveArrayTime"
              placeholder="请选择约定到达时间"
              readonly
              class="textRight"
              @click="reserveArrayTimeShow = true"
            />
            <!-- 选择约定到达时间 -->
            <van-popup
              v-model="reserveArrayTimeShow"
              position="bottom"
              :style="{ height: '40%' }"
            >
              <van-datetime-picker
                v-model="reserveArrayTimeCurrent"
                type="datetime"
                :min-date="minDate"
                :max-date="maxDate"
                :formatter="formatter"
                cancel-button-text="开始时间"
                confirm-button-text="完成"
                @confirm="confirmReserveArrayTime"
              />
            </van-popup>
          </div>
          <!-- 实际到达时间 -->
          <div class="region">
            <van-field
              ...
              is-link
              arrow-direction="down"
              v-model="xiaoJiaoRealityArrayTime"
              placeholder="请选择实际到达时间"
              readonly
              class="textRight"
              @click="realityArrayTimeShow = true"
            />
            <!-- 选择约定到达时间 -->
            <van-popup
              v-model="realityArrayTimeShow"
              position="bottom"
              :style="{ height: '40%' }"
            >
              <van-datetime-picker
                v-model="realityArrayTimeCurrent"
                type="datetime"
                :min-date="minDate"
                :max-date="maxDate"
                :formatter="formatter"
                cancel-button-text="开始时间"
                confirm-button-text="完成"
                @confirm="confirmRealityArrayTime"
              />
            </van-popup>
          </div>
        </template>
      ...
      <template v-else>
        ...
        <div class="region" v-if="workType == 5012 && solution1 == 15">
          <van-field
            ...
            is-link
            arrow-direction="down"
            v-model="rbBaseStationEnterTime"
            placeholder="请选择复建站点入网时间"
            readonly
            class="textRight"
            @click="rbBaseStationEnterTimeShow = true"
          />
          <!-- 选择复建站点入网时间 -->
          <van-popup
            v-model="rbBaseStationEnterTimeShow"
            position="bottom"
            :style="{ height: '40%' }"
          >
            <van-datetime-picker
              v-model="rbBaseStationEnterTimeCurrent"
              type="datetime"
              :min-date="minDate"
              :max-date="maxDate"
              :formatter="formatter"
              cancel-button-text="开始时间"
              confirm-button-text="完成"
              @confirm="confirmRbBaseStationEnterTime"
            />
          </van-popup>
        </div>
      </template>
...
export default {
  ...
  data () {
    return {
      ...
      // 故障恢复时间
      excludeTime: '',
      excludeTimeShow: false,
      excludeTimeCurrent: new Date(),
      ...
      fgsCoordinate: 0, // 是否需分公司配合 -1：该内容页面上不显示；0：否，1：是
      ...
      dealDate: '', // 处理时间 
      // 时间选择
      processTimeShow: false,
      minDate: new Date(2000, 0, 1),
      maxDate: new Date(2090, 10, 1),
      dealDateCurrent: new Date(),
      linkTel: '', // 联系电话
      ...
      reserveArrayTimeShow: false,
      reserveArrayTimeCurrent: new Date(),
      xiaoJiaoRealityArrayTime: '', // 实际到达时间
      realityArrayTimeShow: false,
      realityArrayTimeCurrent: new Date(),
      finishInfo: '', // 回复信息
      ...
      rbBaseStationEnterTimeShow: false,
      rbBaseStationEnterTimeCurrent: new Date(),
  ...
  methods: {
    ...
    clickIsMainDept (bool) {
      if (bool) this.isMainDept = '1'
      else this.isMainDept = '2'
    },
    // 选中是否误报
    ...
    // 选择处理时间
    confirmProcessTime (value) {
      this.dealDate = this.dataFormat(value)
      this.processTimeShow = false
    },
    // 选择约定到达时间
    confirmReserveArrayTime (value) {
      this.xiaoJiaoReserveArrayTime = this.dataFormat(value)
      this.reserveArrayTimeShow = false
    },
    // 选择实际到达时间
    confirmRealityArrayTime (value) {
      this.xiaoJiaoRealityArrayTime = this.dataFormat(value)
      this.realityArrayTimeShow = false
    },
    // 选择故障恢复时间
    confirmExcludeTime (value) {
      this.excludeTime = this.dataFormat(value)
      this.excludeTimeShow = false
    },
    //  选择复建站点入网时间
    confirmRbBaseStationEnterTime (value) {
      this.rbBaseStationEnterTime = this.dataFormat(value)
      this.rbBaseStationEnterTimeShow = false
    },
    // 新增硬件
    addHardWare () {
    ...
    confirmAddHardWarePop () {
      if (this.changeHardwareType == '') return this.$toast('更换硬件类型不能为空')
      if (this.changeHardwareVersion == '') return this.$toast('更换硬件型号不能为空')
      if (this.changeHardwareNumber == '') return this.$toast('更换硬件数量不能为空')
      let obj = {
      ...
      this.addHardWarePopShow = false
      this.changeHardwareType = this.changeHardwareVersion = this.changeHardwareNumber = ''
      ...
      if (this.workType == 5009) {
        ...
        if (this.solution1 == 5) {
          ...
          if (this.solution2 == 58) {
            ...
            this.uploadProof1 = true
            this.uploadProof2 = true
            this.proof1 = '证据1：(物业/涉密单位不同意书面证明（含盖章或签字）或录音证据+投诉用户解释录音)'
            this.proof2 = '证据2：(物业/涉密单位不同意书面证明（含盖章或签字）或录音证据+投诉用户解释录音)'
        ...
        if (this.solution1 == 18) {
          ...
          if (this.solution2 == 61) {
            ...
            this.uploadProof1 = true
            this.uploadProof2 = true
            this.proof1 = '证据1：(物业/涉密单位不同意书面证明（含盖章或签字）)'
            this.proof2 = '证据2：(现场安装照片)'
          }
        }
    ...
    async getFinishQuery () {
      let result = await reqFinishQuery(JSON.stringify({ id: parseInt(this.$route.query.id) }))
      ...
      if (result.operationSuccessFlag) {
        ...
        if (this.hasWorkType) {
          ...
          // 故障分类-控制页面显示
          let isMainDeptId = [14, 15, 16, 17, 18, 29, 5001, 5002, 5003, 5004, 5005, 5006]
          ...
          if (result.balkSubSortId == 20) {
            ...
            let currentDeptList = [5747, 5776, 5793, 5814, 5862, 6043, 6059, 6076, 6094, 6138, 6182, 6207, 6248, 6260, 6306, 6317]
            ...
            // 显示-faultFeedback；faultFeedbackInfo；dealDate；verificationResult
          ...
          if (result.xiaoJiaoBackInfoList.length > 0) {
            ...
            let xiaoJiaoBackInfo = result.xiaoJiaoBackInfoList[result.xiaoJiaoBackInfoList.length - 1]
            ...
            // 故障原因类型
            this.xiaoJiaoBalkReasonType = xiaoJiaoBackInfo.xiaoJiaoBalkReasonType.toString()
    ...
    async submitFinish () {
      ...
      let postData = {
        ...
        dealGroupId: this.currWorkGroupInfo.groupId.toString(), // 处理部门Id
        dealGroupName: this.currWorkGroupInfo.groupName, // 处理部门名称
        excludeTime: this.excludeTime,
        isMainDept: this.isMainDept,
        faultReasonLevel1: this.faultReasonLevel1,
        faultReasonLevel2: this.faultReasonLevel2,
        faultReasonLevel3: this.faultReasonLevel3,
        ttStation: this.ttStation == 2 ? 'true' : 'false',
    ...
    receiptCheck () {
      ...
      if (this.hasWorkType) {
        ...
        if (this.workType == 5008 && (this.solution1 == 1 || this.solution1 == 2)) {
          if (this.prb == '') return this.$toast('请输入基站忙时PRB利用率')
        }

        if (this.workType == 5011 && this.solution1 == 12) {
          if (this.baseStationStatus == '') return this.$toast('请输入基站指标情况')
        }

        if (this.workType == 5012 && this.solution1 == 15) {
          if (this.rbBaseStationName == '') return this.$toast('请输入复建站点名称')
          if (this.rbBaseStationEnterTime == '') return this.$toast('请选择复建站点入网时间')
        }
        ...
        if (this.balkSubSortId20Show || this.balkSubSortId21Show || this.balkSubSortId22Show) {
          if (this.dealDate == '') return this.$toast('请选择处理时间')
          if (this.faultFeedbackInfo == '') return this.$toast('请输入排障反馈详情')
        }

        if (this.isXiaoJiao) {
          if (this.xiaoJiaoBrand == -1) return this.$toast('请选择小交品牌')
          if (this.xiaoJiaoBalkReason == '') return this.$toast('请输入故障原因')
          if (this.xiaoJiaoBalkReasonType == '') return this.$toast('请选择故障原因类型')
          if (this.xiaoJiaoProcMode == '') return this.$toast('请输入处理方式')
          if (this.xiaoJiaoBalkGrade == -1) return this.$toast('请选择故障等级')
          if (this.xiaoJiaoServiceMan == '') return this.$toast('请输入现场维修人员')
          if (this.xiaoJiaoReserveArrayTime == '') return this.$toast('请选择约定到达时间')
          if (this.xiaoJiaoRealityArrayTime == '') return this.$toast('请选择实际到达时间')
        }
    ...
    getPictureIds () {
      ...
      if (this.imgList.length > 0) {
        this.imgList.forEach((item, index) => {
          ...
          formData.append("picName", `${this.$route.query.id}-${index}`)
