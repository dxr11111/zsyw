      ...
      <van-form>
        <div>
          ...
          <van-cell style="padding: 0">
            ...
            <table  v-if="engineerInfoList?.length > 0" class="table" border="1" cellspacing="0" width="100%">
              <tr>
                <th>姓名</th>
                <th>电话</th>
                <th>在岗状态</th>
...
export default {
  ...
  created () {
    GetAssignAccendantApi(JSON.stringify({ id: Number(this.$route.query.id) })).then(res => {
  ...
  methods: {
    ...
    addEngineer (type, item) {
      ...
      if (type == 'hand') {
        ...
        let reg = /(\d{7,8})|(^(0|86|17951)?(13[0-9]|15[012356789]|166|17[3678]|18[0-9]|14[57])[0-9]{8}$)/
        if (!reg.test(this.phoneNumber)) return this.$toast('电话格式不正确')
        params = {
          id: 0,
          userName: this.userName,
          phoneNumber: this.phoneNumber,
          address: this.addressInput || this.currAddress
        }
        ...
        this.engineerInfoList = this.engineerInfoList.filter(e => e.userId !== item.userId)
      }
      this.currSelectedEngineerList.push(params)
    },
    // 删除维护人
    delEngineer (e, i) {
      ...
      this.engineerInfoList = this.list.filter(item => item.userId == e.userId)
    ...
    async onSubmit () {
      ...
      let data = await AddAssignAccendantApi(JSON.stringify({ id: Number(this.$route.query.id),appointRepairInfoList: arr }))
