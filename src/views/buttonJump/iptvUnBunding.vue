...
<script>
import { reqIomNewIptvUnBundingQuery, reqIomNewIptvUnBunding } from '@/http/button';
...
export default {
  ...
  methods: {
    ...
    async getIomNewIptvUnBundingQuery () {
      let result = await reqIomNewIptvUnBundingQuery(JSON.stringify({ id: parseInt(this.$route.query.id) }))
    ...
    async sendIptvUnBunding (iptvnbr) {
      ...
      let result = await reqIomNewIptvUnBunding(JSON.stringify({ id, iptvNbrs }))
