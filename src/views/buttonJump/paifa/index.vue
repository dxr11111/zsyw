...
<script>
import { GetPaiFaQueryApi, GetPaiFaQueryKzwwApi, PaiFaConfirmApi } from '@/http/button.js'
  ...
  methods: {
    async submit () {
      ...
      if (this.paifaJsonList.length > 0) {
        if (this.currDept == '请选择') return this.$toast('请选择网维故障处理专业')
        params.choseSpec = this.paifaJsonList.find(e => e.name == this.currDept).id
      }
    ...
    async getSeclected (data) {
      ...
      this.selectList.forEach(e => {
        ...
        if (str === '网络维护中心' || str === '接入网支撑网格') this.specialList.push(e)
      })
      // 只查询特殊列表中的第一个部门数据并展示
      let res = await GetPaiFaQueryKzwwApi(JSON.stringify({ id: this.$route.query.id, groupId: this.specialList[0]?.paifaFlowNode.groupId }))
      console.log('special', res)
      this.paifaJsonList = res.paiFaJsonList.map(e => ({ name: e.value, id: e.key }))
    },
    // 派发前查询
    async getPaifaData () {
      let data = await GetPaiFaQueryApi(JSON.stringify({ id: Number(this.$route.query.id), isFirstPaifa: 2 }))
