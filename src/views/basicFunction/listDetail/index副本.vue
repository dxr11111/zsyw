<template>
  ...
  <div class="listDetail">
    <div class="static" ref="static">
      ...
      <ul class="tab">
        <li
          ...
          @click="changeTab(base.key, index)"
        >
          {{ base.key }}
        </li>
      </ul>
    ...
    <!-- tab栏下的content部分 -->
    <ul class="tabContent" :style="{ marginTop: tabMarginTop }">
      <li
        v-for="(base, index) in baseInfo"
        :key="index"
    ...
    <div
      ...
      v-if="basicInfoAttachmentInfoList.length > 0 && tabActive == 0"
    >
      <div class="title">附件信息</div>
      <div class="content">
        <span v-for="atta in basicInfoAttachmentInfoList" :key="atta.fileId">{{
          atta.fileName
        }}</span>
      </div>
    </div>
    <!-- 点击截屏 -->
    <div class="screenShot" @click="saveImage" v-show="tabActive == 0">
...
export default {
  ...
  data() {
    return {
      ...
      basicInfoAttachmentInfoList: [], // 局内工单附件信息
    };
  },
  provide() {
      ...
      try {
        ...
        if (getItem("workOrderDetail")?.sysId == 11) {
          ...
          if (this.listDetail.operationSuccessFlag) {
            ...
            // 局内工单在基本信息最末尾展示附件信息basicInfoAttachmentInfoList
            if (this.listDetail?.basicInfoAttachmentInfoList) {
              this.basicInfoAttachmentInfoList =
                this.listDetail.basicInfoAttachmentInfoList;
            }
          } else {
            // 请求失败
  ...
  mounted() {
    ...
    this.tabMarginTop = this.$refs.static.offsetHeight + 54 + "px";
  },
  created() {
    // 获取列表详情
    this.getListDetail();
