    ...
    <div class="static">
      <div class="head" ref="head" :style="{ height: headHeight }">
        <div class="headText" :style="{ margin: headTextMargin }">
          <div class="left">
            <!-- 任务-显示用户名 -->
            <div class="userName" v-if="isTask">
...
export default {
  name: 'WorkBench',
  components: { ContentInfo, Filtrate, MyTask, DepartmentOrder, JsMiddlePlatform, PhoneIcon },
  ...
  data () {
    return {
      ...
      orderId: -1, //  4：局内 5：基站 6：建设   ifm维护类（31:故障单 33:预警单 34:支撑单 35:重保单 36:优化单 ） 7：局内 9：基站 10：新装机单
      orderName: '',
  ...
  methods: {
    ...
    clickMenu (bool) {
      if (bool == 0) {
        ...
        // 交集
        let unite = sysIds.filter((item) => { return [3, 7, 9].indexOf(item) > -1 })
        if (unite.length > 0) {
          // sysIds内存在3,7,9，如果只有3且ifmSysIds无值，则没有工单
          if (unite.length == 1 && unite[0] == 3) {
            // 判断ifmSysIds
            if (this.getLoginInfo?.ifmSysIds.length == 0) return this.$toast('您没有部门工单的操作权限')
          }
        } else {
    ...
    clickSort (value) {
      ...
      if (listSort.currentId == value) {
        if (listSort.actions[value - 1].arrows == 'icon-xiajiantou') {
          // 取反
          this.$set(this.listSort.actions[value - 1], 'arrows', 'icon-shangjiantou')
          this.$set(this.listSort, 'sort', 1)
        } else {
          this.$set(this.listSort.actions[value - 1], 'arrows', 'icon-xiajiantou')
    ...
    sysId为3的找登录返回的ifmSysIds（维护类）在工单模块显示，
      ...
      if ((getItem('jsMiddlePlatformFlag'))?.isLocation && (getItem('jsMiddlePlatformFlag'))?.isCreate) {
        let order = {
          id: 6,
          name: "建设中台",
          sysId: 11,
        ...
        for (let action of this.selectOrderActions) {
          if (action.content.length > 0) {
            return this.selectOrder(action.content[0])
          }
        }
      }
      ...
      if (groupId !== -1) {
        this.getLoginInfo.listSheetGroup.forEach(item => {
          if (item.groupId === groupId) {
            ...
            // 存入vuex
            this.$store.commit('workOrder/updateCurrWorkGroupInfo', { groupId, groupName })
          }
        })
      }
    },
    ...
    monitorMainScroll () {
      // 监听div滚动事件
      let self = this
      this.$parent.$refs.main.addEventListener('scroll', this.scrolling.bind(this, self))
    ...
    judgePhoneIcon () {
      ...
      // 获取交集
      let unite = arr.filter(function (val) { return sysIds.indexOf(val) > -1 })

      if (this.isTask && unite.length > 0) {
        return true
      } else {
        false
      }
      ...
      if (id) {
        ...
        this.$nextTick(() => {
          ...
          this.tabList.forEach((e, i) => {
            if (e.id == id) {
              setTimeout(() => { this.tabActive = i }, 500)
            }
          })
          this.resetListInfo()

        })
        console.log(`首页参数跳转任务tab,id:${id},tabActive:${this.tabActive},taskType:${this.taskType} `);
        removeItem('taskId')
      }
    },
