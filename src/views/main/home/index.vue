...
import { reqgetTools, reqgetTaskBoard, reqgetUnReadMessageNum, reqEveryDayStudy } from '@/http/index'
import { billList } from '@/utils/workIcons.js'
import { matchTools } from '@/utils/tools'
export default {
  name: 'Home',
  data () {
    return {
  ...
  methods: {
    ...
    closeEveryDayStudy () {
      ...
      localStorage.setItem('everyDayStudy',
        JSON.stringify({
          time,
    ...
    // 获取首页任务看板
    async getTaskList () {
      let data = await reqgetTaskBoard(JSON.stringify({ sysIds: this.sysIds, homeTaskType: '', ifmSysIds: this.ifmSysIds }))
    ...
    // 根据权限获取工具列表
    async getTools () {
      let data = await reqgetTools(JSON.stringify({ sysIds: this.sysIds, ifmSysIds: this.ifmSysIds }))
      ...
      if (list?.length > 0) {
        this.toolsList = []
        // 当前用户的排序列表
        let curr = list.find(e => getItem('loginInfo').loginNo == e.loginNo)?.sortList
        if (curr?.length > 0) {
    ...
    async getMessageList () {
      let data = await reqgetUnReadMessageNum(JSON.stringify({ sysIds: this.sysIds }))
