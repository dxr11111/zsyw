  ...
  <div>
    <div class="top">
      ...
      <div class="info">
        ...
        <div class="count" v-if="mobileNumber?.length > 0">手机号：{{ mobileNumber }}</div>
      </div>
    </div>
    <div>
      <van-cell :value="quickLoginType" is-link @click="$router.push('/quickLogin')">
        <template #title>
...
import { GetPushStatusApi, SendFeedBackApi, CheckVersionApi, LogoutApi, GetPhoneNumberApi } from '@/http/index'
export default {
  name: 'My',
  data () {
    return {
...
.top {
  height: 283px;
  background-image: url('@/assets/imgs/my-top.png');
