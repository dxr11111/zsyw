  ...
  methods: {
      ...
      this.apiResponse(result, ".selectMan", () => {
        if (this.showFlag == 1) {
          ...
          if (result.diagnoseReasonList.length > 0) {
            ...
            result.diagnoseReasonList.length == 0 &&
            result.dwGroupList.length == 0
          ) {
            // 3. 无诊断原因，无派修人
    ...
    getInfo() {
      ...
      if (this.$route.query.piaTypeDiagnoseResult) {
        this.piaTypeDiagnoseResult = JSON.parse(
          this.$route.query.piaTypeDiagnoseResult
        );
      }
