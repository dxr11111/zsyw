    ...
    <div class="main">
      <van-form @submit="onSubmit">
        ...
        <van-field
          ...
          class="marginTop"
          v-if="errorReasonCode === '90010006' || type == 2"
        />
        <!-- 确定 -->
        <div class="bottom">
...
export default {
  ...
  computed: {
    ...mapState("returnSheet", ["returnSheet", "type"]),
    headName() {
      return this.type == 1
        ? `退工单(${this.$route.query.orderNum})`
        : `修改标准地址(${this.$route.query.orderNum})`;
    },
  },
  methods: {
    // 回退
    ...
    async onSubmit() {
      // 表单校验
      if (this.type == 1) {
        if (this.explain == "") return this.$toast("请输入退单说明");
        if (this.errorReasonCode === "90010005") {
          if (this.soNbr == "")
            return this.$toast("请输入已竣工宽带编码或订单号");
        }
      }
      if (this.errorReasonCode === "90010006" || this.type == 2) {
        if (this.addrKeyword == "") return this.$toast("请输入标准地址关键字");
      }
      ...
      if (this.type == 1) {
        ...
        if (this.errorReasonCode === "90010006") {
          ...
          this.apiResponse(result, ".returnSheetMain", () => {
            // 接口按钮操作成功 刷新工作台
