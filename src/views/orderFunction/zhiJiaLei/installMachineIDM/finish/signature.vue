...
export default {
  ...
  methods: {
    ...
    async onSubmit() {
      ...
      let signIds = this.signIds; // 签字图片id
      let imageIdList = this.imageIdList; // 外线回单用户现场图片id-回复
      let postData = this.$route.query.postData; // 强制回单页面传过来的请求参数
      postData = { ...postData, satisfied, isBuildImage, signIds, imageIdList };
      console.log(postData);

      try {
        let result = await reqIomNewFinish(JSON.stringify(postData));
            ...
            this.buildImageList.forEach((item, index) => {
              ...
              imgRequests({
                ...
                .then((res) => {
                  console.log("图片id结果", res);
                  // 获取图片id
                  this.imageIdList.push(parseInt(res.id));
                  // 判断如果是最后一次图片请求，则发送强制回单/回复 请求
                  if (this.buildImageList.length === this.imageIdList.length) {
                    // 发送强制回单/回复 请求
                    this.onSubmit();
                  }
                })
