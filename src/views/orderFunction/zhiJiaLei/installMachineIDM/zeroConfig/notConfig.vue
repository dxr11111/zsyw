...
export default {
  ...
  methods: {
    ...
    // 表单校验
    checkForm() {
      if (this.accessWay === "FTTB" || this.accessWay === "FTTH") {
      ...
      switch (this.accessWay) {
          ...
          this.$dialog
            ...
            .then(async () => {
              ...
              try {
                ...
                let result = await reqIomNewZeroConfigFtth(
        ...
        case "FTTB":
          if (this.submitText === "提交") {
            ...
            let submitResult = await reqIomNewZeroConfigFttb(
              JSON.stringify(postData)
            );
          } else {
            // 查询端口
            ...
            let queryResult = await reqIomNewQueryFttbPort(
              JSON.stringify({ id, mac, sn })
            );
            console.log("查询端口结果", queryResult);
            this.apiResponse(queryResult, ".notConfig", () => {
              let dataFttbPortList = JSON.stringify(
                queryResult.dataFttbPortList
              );
              let voiceFttbPortList = JSON.stringify(
                queryResult.voiceFttbPortList
              );
              // 跳转到选择端口页面
              this.$router.push({
                name: "ChoosePort",
        ...
        case "AG":
          if (this.submitText === "提交") {
            ...
            let ipInfo = "";
            let portInfo = "";
            let submitResult = await reqIomNewZeroConfigAg(
              JSON.stringify({ id, accessMode, ipInfo, portInfo })
            );
            ...
            let queryResult = await reqIomNewQueryAgPort(
              JSON.stringify({ id, agIp, startPort, stopPort })
            );
        ...
        case "PON+D":
          ...
          let ponDresult = await reqIomNewZeroConfigPonD(
            JSON.stringify({ id, accessMode, exchSystemCode })
          );
          console.log("PON+D提交结果", ponDresult);
          this.apiResponse(ponDresult, ".notConfig", () => {});
          break;
        case "ADSL":
          // 提交
          let adslResult = await reqIomNewZeroConfigAdsl(
            JSON.stringify({ id, accessMode })
          );
          console.log("ADSL提交结果", adslResult);
          this.apiResponse(adslResult, ".notConfig", () => {});
          break;
      }
    },
    // 选中接入方式
    async selectAccessWay(action) {
      ...
      if (this.businessTypeCode === 1) {
        ...
        if (this.accessWay === "PON+D") {
          // 调用接口获取交换系统号内容
          try {
            let result = await reqIomNewQueryExchSystemCode(
