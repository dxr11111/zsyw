...
<script>
import { reqIomPosInfoQuery } from "@/http/button";
...
export default {
  ...
  data() {
    return {
      headName: `POS信息(${this.$route.query.orderNum})`,
      posInfo: [],
  ...
  methods: {
    ...
    async getPosInfo() {
      ...
      let posId = "";
      let result = await reqIomPosInfoQuery(
        JSON.stringify({ id, mac, sn, posId })
      );
      console.log("pos查询结果", result);
      this.apiResponse(result, ".posInfo", () => {
