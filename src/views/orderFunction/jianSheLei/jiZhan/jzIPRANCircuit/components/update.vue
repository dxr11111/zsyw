...
export default {
  ...
  data() {
    return {
      ...
      headName: `修改连接位置(${this.updateInfo.weizhiMiaoShu})`,
      deviceNames: [],
      moduleNames: [],
      connPositionSource: [],
  ...
  created() {
    // this.headName = `修改连接位置(${this.currData.weizhiMiaoShu})`
    var arr = this.updateInfo.weiZhiId.split("/")
  ...
  methods: {
    onSubmit() {
      ...
      this.params = {
        ...
        qianShebeilxId: this.updateInfo.shebeiLeiXingId, // 前设备类型id
        qianWeiZhiId: this.updateInfo.weiZhiId, // 前连接位置id
        qianShebeiId: this.updateInfo.shebeiId, // 前设备id
        qianMokuaiId: this.updateInfo.mokuaiId, // 前模块id
    ...
    async getSpeclineRoute() {
      let data = await JndSpecLineRouteTerminalApi(
        JSON.stringify({
          shebeiLeiXingId: this.updateInfo.shebeiLeiXingId,
          weiZhiId: this.updateInfo.weiZhiId,
          weizhiMiaoShu: this.updateInfo.weizhiMiaoShu,
        })
      )
      // console.log(data)
