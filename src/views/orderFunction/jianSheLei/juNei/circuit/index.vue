<template>
  ...
  <div class="circuit">
        ...
        <div class="details">
          ...
          <div class="buttonRegion">
            <van-button
              ...
              :key="buttonIndex"
              @click="clickButton(button, item['DL_ID'], item['电路编号'])"
              >{{ button }}</van-button
            >
          </div>
        </div>
    ...
    >
      <h4>{{ popText }}</h4>
      <van-cell>确认电路编号：{{ circuitNum }}</van-cell>
      <van-field
        v-model="intro"
        :label="popText + '说明'"
...
import {
  reqJnddQueryCircuit,
  reqJnddCircuitBack,
  reqJnddCircuitFinish,
  reqJnddCircuitRejectConfirm,
} from "@/http/button";
...
export default {
  ...
  data() {
    return {
      ...
      popShow: false,
      intro: "",
      circuitid: [], // 电路编号'DL_ID'
      circuitNum: "", // 当前选中的电路编号
      // 退电路，回复，确认驳回的弹出层内容
      popText: "",
    };
  },
  ...
  methods: {
    ...
    // 点击任意一项的按钮
    clickButton(buttonName, circuitid, circuitNum) {
    ...
    async confirmPop() {
      ...
      let result = null;
      if (this.popText == "退电路") {
        result = await reqJnddCircuitBack(
          JSON.stringify({ id, circuitid, intro })
        );
      } else if (this.popText == "回复") {
        result = await reqJnddCircuitFinish(
          JSON.stringify({ id, circuitid, intro })
        );
      } else if (this.popText == "确认驳回") {
        result = await reqJnddCircuitRejectConfirm(
          JSON.stringify({ id, circuitid, intro })
        );
      }
      console.log("请求结果", result);
      this.apiResponse(result, ".circuit", () => {
        // 再次查询电路信息
        this.getCircuitInfo();
      });
      this.closePop();
    ...
    async getCircuitInfo() {
      let result = await reqJnddQueryCircuit(
      ...
      this.apiResponse(result, ".circuit", () => {
        ...
        // 将编号替换为对应中文
        this.circuitList = this.getNewCircuitList(
          JSON.parse(JSON.stringify(result.circuit))
        );
      });
    },
    // 获取下拉菜单距离顶部位置
    ...
    confirmMenu() {
      ...
      // 获取完整的circuitList
      let allCircuitList = this.getNewCircuitList(
        JSON.parse(JSON.stringify(this.apiCircuitList))
      );

      if (
        this.circuitBuildText.length > 0 &&
        this.orderCircuitText.length > 0
      ) {
        // 两个都有选择
        this.circuitList = [];
        this.apiCircuitList.forEach((info, index) => {
          if (
            activeAll.includes(parseInt(info["电路施工状态"])) &&
            activeAll.includes(parseInt(info["工单电路状态"]))
          ) {
            // 渲染符合规则的信息
            let tempList = allCircuitList.filter(
              (currentValue, currentIndex, currentArr) => {
