...
// 根据需求压缩图片
export const getCompressFile = function (fileObj, isHalf) {
      ...
      reader.onload = function (e) {
        ...
        image.onload = function () {    //图片加载完毕后再通过canvas压缩图片，否则图片还没加载完就压缩，结果图片是全黑的
          let canvas = document.createElement('canvas'), //创建一个canvas元素
            ...
            imageWidth = isHalf ? image.width / 2 : image.width * (0.75),    //压缩后图片的宽度
            imageHeight = isHalf ? image.height / 2 : image.height * (0.75),    //压缩后图片的高度
            data = ''    //存储压缩后的图片
